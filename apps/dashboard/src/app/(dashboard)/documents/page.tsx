...
export default function DocumentsPage() {
  ...
  const createMutation = useMutation({
    mutationFn: () => api.post('/documents', { name, type, fileUrl }),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ['documents'] });
      toast.success('Document created');
      setShowForm(false); setName(''); setType(''); setFileUrl('');
    },
    ...
    <div className="h-full flex flex-col">
      ...
      {showForm && (
        <div className="border-b border-gray-200 bg-white p-3 space-y-2 shrink-0">
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Document name (required)" className="w-full border border-gray-200 rounded px-2.5 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-violet-400" />
          <select value={type} onChange={(e) => setType(e.target.value)} className="w-full border border-gray-200 rounded px-2.5 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-violet-400">
            <option value="">Select type...</option>
            <option value="proposal">Proposal</option>
            <option value="contract">Contract</option>
            <option value="other">Other</option>
          </select>
          <input value={fileUrl} onChange={(e) => setFileUrl(e.target.value)} placeholder="File URL" className="w-full border border-gray-200 rounded px-2.5 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-violet-400" />
