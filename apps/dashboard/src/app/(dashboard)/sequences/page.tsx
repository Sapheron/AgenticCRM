...
export default function SequencesPage() {
  ...
  const createMutation = useMutation({
    mutationFn: () => api.post('/sequences', { name }),
    onSuccess: () => {
    ...
    <div className="h-full flex flex-col">
      ...
      {showForm && (
        <div className="border-b border-gray-200 bg-white p-3 space-y-2 shrink-0">
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Sequence name (required)" className="w-full border border-gray-200 rounded px-2.5 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-violet-400" />
