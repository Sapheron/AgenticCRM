# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║          AgenticCRM — Windows Installer (PowerShell)                    ║
# ║          https://agenticcrm.sapheron.com                                  ║
# ║                                                                              ║
# ║  Run:  powershell -c "irm https://agenticcrm.sapheron.com/install.ps1 | iex"║
# ╚══════════════════════════════════════════════════════════════════════════════╝
...
# ── Config ────────────────────────────────────────────────────────────────────
$InstallDir   = if ($env:INSTALL_DIR) { $env:INSTALL_DIR } else { "C:\agenticcrm" }
$RepoUrl      = if ($env:REPO_URL)    { $env:REPO_URL    } else { "https://github.com/Sapheron/AgenticCRM.git" }
...
Write-Host "  ║        AgenticCRM — Installer v1.0            ║" -ForegroundColor Cyan
Write-Host "  ║    AgenticCRM  •  Self-hosted  •  Windows   ║" -ForegroundColor Cyan
Write-Host "  ╚═══════════════════════════════════════════════════╝" -ForegroundColor Cyan
Write-Host ""
...
if (-not $WslInstalled) {
  Warn "WSL2 not found — AgenticCRM runs best on WSL2"
...
$ImagesExist = (docker images --format "{{.Repository}}" 2>$null | Select-String "agenticcrm").Count
...
Write-Host "  ║   OK  AgenticCRM is ready!                       ║" -ForegroundColor Green
Write-Host "  ╚══════════════════════════════════════════════════════╝" -ForegroundColor Green
Write-Host ""
...
Write-Host "  powershell -c `"irm https://agenticcrm.sapheron.com/install.ps1 | iex`"" -ForegroundColor Cyan
Write-Host ""
